...
```{r}
# clean up the above cod eo tmake it cleaner, key functions: logistic_reg()
# set_engine(), fit(), predict(), bind_cols(), metrics()
...
```{r}
# create trianing data and test on training data

set.seed(10)
split_result <- clean %>% initial_split(prop = 0.01)

training_data <- training(split_result)
testing_data <- testing(split_result)

logistic_reg() %>%
  set_engine("glm") %>%
  fit(dvote ~ gender + party + state + office, data = training_data) %>% 
